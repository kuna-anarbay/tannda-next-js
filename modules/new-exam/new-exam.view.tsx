...
interface NewExamViewProps {
    ...
    sections: Section[];
    handleSubmit: (body: CreateExamRequestDto) => void;
    handleFile: (e) => void;
    removeFile: (index: number) => void;
    files: Resource[];
}

export default function NewExamView(props: NewExamViewProps) {
    const {sections, files, loading, defaultSectionId, open, close, handleSubmit, handleFile, removeFile} = props;

    const initialValues: CreateExamRequestDto = {
        title: "",
        description: "",
                    ...
                    {({values}) => (
                        <Form className={"form text-label fixed top-0 bottom-0 py-8 w-1/2 left-1/4 z-40"}>
                            <div className={"bg-background h-full rounded-1.5 space-y-4 relative"}>
                                <div
                                    className={"relative flex items-center justify-center px-4 border-b border-border"}>
                                    <div onClick={close}
